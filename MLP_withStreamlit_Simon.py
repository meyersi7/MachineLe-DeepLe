...
# ===========================================================================================
st.set_page_config(page_title="MLP Dashboard", page_icon="🧠", layout="wide")
st.title("🧠 MLP Binary Classification Dashboard")
...
data_source = st.sidebar.radio(
    "Datenquelle wählen:",
    ["Simulierte Daten", "CSV hochladen"],
)
...
if data_source == "CSV hochladen":
    ...
    if uploaded_file is not None:
        ...
        st.info("Bitte eine CSV-Datei hochladen oder simulierte Daten wählen.")
        st.stop()
else:
...
# ===========================================================================================
# Sidebar — Zielvariable und Features wählen
# ===========================================================================================
st.sidebar.header("🎯 Variablen")

target_col = st.sidebar.selectbox("Zielvariable (binär):", df.columns, index=len(df.columns) - 1)
...
# ===========================================================================================
# Sidebar — MLP Hyperparameter
# ===========================================================================================
st.sidebar.header("⚙️ MLP Hyperparameter")

n_layers = st.sidebar.slider("Anzahl Hidden Layers", 1, 5, 2)
neurons_per_layer = []
for i in range(n_layers):
    n = st.sidebar.slider(f"Neuronen Layer {i + 1}", 8, 256, max(64 // (2 ** i), 16), step=8)
...
dropout_rate = st.sidebar.slider("Dropout", 0.0, 0.5, 0.2, step=0.05)
batch_size = st.sidebar.select_slider("Batch Size", options=[16, 32, 64, 128, 256], value=64)
num_epochs = st.sidebar.slider("Epochen", 10, 500, 100, step=10)
test_size = st.sidebar.slider("Test-Anteil", 0.1, 0.4, 0.2, step=0.05)

# ===========================================================================================
# Daten-Vorschau
# ===========================================================================================
st.header("📊 Daten-Vorschau")
col1, col2 = st.columns(2)
with col1:
    st.dataframe(df.head(10), use_container_width=True)
with col2:
    st.write("**Shape:**", df.shape)
    st.write("**Target-Verteilung:**")
...
# ===========================================================================================
# MLP Modell — dynamisch nach Sidebar-Einstellungen
# ===========================================================================================
def build_mlp(input_size, neurons_per_layer, dropout_rate):
    """Dynamisches MLP basierend auf Sidebar-Einstellungen."""
    layers = []
    in_size = input_size
...
if st.button("▶️ Modell trainieren", type="primary", use_container_width=True):
    ...
    with st.expander("Modell-Architektur"):
    ...
    status_text.text(f"✅ Training abgeschlossen in {train_time:.1f}s")
    ...
    col_acc, col_f1, col_auc = st.columns(3)
    ...
    col_acc.metric("Accuracy", f"{accuracy * 100:.1f}%")
    col_f1.metric("F1-Score", f"{f1:.3f}")
    col_auc.metric("AUC-ROC", f"{roc_auc:.3f}")
    ...
    with st.spinner("Permutation Importance wird berechnet..."):
    ...
    col_pi_chart, col_pi_table = st.columns([2, 1])
    ...
    # Modell in Session State speichern für Optuna-Vergleich
    st.session_state["manual_f1"] = f1
    st.session_state["manual_model"] = model

    st.success("✅ Training und Evaluation abgeschlossen!")

# ===========================================================================================
# OPTUNA — Automatische Hyperparameter-Suche
# ===========================================================================================
st.header("🔬 Optuna — Hyperparameter-Suche")

with st.expander("ℹ️ Was macht Optuna?"):
    st.write(
...
n_trials = col_opt1.number_input("Anzahl Trials", min_value=5, max_value=200, value=30, step=5)
optuna_epochs = col_opt2.number_input("Epochen pro Trial", min_value=10, max_value=200, value=50, step=10)

if st.button("🔬 Optuna starten", use_container_width=True):
    import optuna
    ...
    def objective(trial):
        ...
        progress_bar.progress((trial.number + 1) / n_trials)
        status_text.text(f"Trial {trial.number + 1}/{n_trials} — F1: {score:.4f}")
    ...
    status_text.text(f"✅ Optuna abgeschlossen! Bester F1: {study.best_value:.4f}")

    # Ergebnisse anzeigen
    col_best, col_history = st.columns(2)

    with col_best:
        st.subheader("🏆 Beste Hyperparameter")
        st.metric("Bester F1-Score", f"{study.best_value:.4f}")

        if "manual_f1" in st.session_state:
            diff = study.best_value - st.session_state["manual_f1"]
            st.metric("Vergleich zu manuellem Modell", f"{diff:+.4f} F1")
    ...
    with st.expander("📐 Beste Modell-Architektur"):
    ...
    st.success("✅ Optuna Hyperparameter-Suche abgeschlossen!")